...
export default function OwnerDrawingsPage() {
  ...
  // Summary
  const totalSheets = MOCK_DRAWING_SETS.reduce((sum, s) => sum + s.sheet_count, 0);
      ...
      {hasData && (
        <div className="grid grid-cols-3 gap-4 mb-5">
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            <div className="text-xs text-gray-500 uppercase tracking-wide">Drawing Sets</div>
            <div className="text-2xl font-bold text-gray-900 mt-1">{MOCK_DRAWING_SETS.length}</div>
          </div>
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            <div className="text-xs text-gray-500 uppercase tracking-wide">Total Sheets</div>
            <div className="text-2xl font-bold text-gray-900 mt-1">{totalSheets}</div>
          </div>
          <div className="bg-white rounded-lg border border-gray-200 p-4">
            <div className="text-xs text-gray-500 uppercase tracking-wide">Current Sets</div>
            <div className="text-2xl font-bold text-green-600 mt-1">
              {MOCK_DRAWING_SETS.filter((s) => s.is_current_set).length}
            </div>
          </div>
        </div>
      )}
