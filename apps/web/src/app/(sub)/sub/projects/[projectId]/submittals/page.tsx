...
const MOCK_SUBMITTALS: Submittal[] = [
  ...
  },
  {
    ...
    days_open: null,
    lead_time_days: 14,
    review_notes: "Duct gauge does not meet spec requirement. Resubmit with 20-gauge per spec.",
...
export default function SubSubmittalsPage() {
  ...
  return (
    <div>
      <PageHeader
        title="Submittals"
        subtitle="Submit and track project submittals"
        action={
          <button
            onClick={() => router.push(`/sub/projects/${projectId}/submittals/new`)}
      ...
      {hasData ? (
        ...
        <EmptyState
          ...
          description="Create a submittal to send materials for review."
          actionLabel="Create Submittal"
          onAction={() => router.push(`/sub/projects/${projectId}/submittals/new`)}
        />
      )}
    </div>
  );
